...
export default function RegretMessagePage() {
  const params = useParams()
  const router = useRouter()
  const messageId = params.id as string

  const message = useMemo(() => regretMessages.find((entry) => entry.id === messageId), [messageId])
      ...
      <main className="min-h-screen flex items-center justify-center p-4">
        <div className="glass-strong rounded-3xl shadow-soft-lg p-6 md:p-10 text-center border border-white/10 max-w-lg w-full bg-white/70 backdrop-blur-xl">
          ...
          <h1 className="text-2xl md:text-3xl font-display font-bold text-gray-800 mb-4">
            This sock went missing!
          </h1>
          <p className="text-base md:text-lg text-gray-600 mb-6">
            We couldn’t find that regret message. Try scanning another sock or return to the start.
          </p>
