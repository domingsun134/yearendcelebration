...
export default function CrackTheCodePage() {
  const params = useParams()
  const router = useRouter()
  const codeId = params.id as string

  const code = useMemo(() => crackTheCodes.find((entry) => entry.id === codeId), [codeId])
      ...
      <main className="min-h-screen flex items-center justify-center p-4">
        <div className="glass-strong rounded-3xl shadow-soft-lg p-6 md:p-10 text-center border border-white/10 max-w-lg w-full bg-white/70 backdrop-blur-xl">
          ...
          <p className="text-base md:text-lg text-gray-600 mb-6">
            We couldn&apos;t find that code. Try scanning another QR code or return to the start.
          </p>
  ...
  return (
    <main className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-br from-yellow-100 via-white to-yellow-100 opacity-70 pointer-events-none"></div>
      <div className="glass-strong rounded-3xl shadow-soft-lg p-6 md:p-10 lg:p-12 text-center border border-white/10 max-w-3xl w-full relative z-10 bg-white/80 backdrop-blur-2xl">
        ...
        <p className="text-base md:text-lg text-gray-600 mb-8">
          Solve the puzzle to unlock the locker! 🎁
        </p>
        <div className="flex justify-center">
          <button
            onClick={() => router.back()}
            className="px-6 py-3 bg-gradient-to-r from-yellow-400 to-yellow-500 text-white rounded-xl font-semibold hover:shadow-glow transition-all duration-300"
          >
            Got it!
          </button>
        </div>
